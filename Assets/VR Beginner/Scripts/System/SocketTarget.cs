...
public class SocketTarget : MonoBehaviour
{
    ...
    {
        var interactor = args.interactorObject as XRExclusiveSocketInteractor;
        
        if(interactor == null)
            return;

        if(SocketType != interactor.AcceptedType)
            return;

        if (DisableSocketOnSocketed)
        {
            //TODO : find a better way, delay feel very wrong
            StartCoroutine(DisableSocketDelayed(interactor));
        }
