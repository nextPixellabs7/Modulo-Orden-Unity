...
public class XROffsetGrabbable : UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable
{
    ...
    {
        // Usar el nuevo método y propiedad recomendados
        var selectInteractor = interactor as UnityEngine.XR.Interaction.Toolkit.Interactors.IXRSelectInteractor;
        if (selectInteractor == null)
            return false;

        int interactorLayerMask = 1 << interactor.gameObject.layer;
        return base.IsSelectableBy(selectInteractor) && (interactionLayers.value & interactorLayerMask) != 0;
    }
}
